    export billiard_rectangle, billiard_sinai, billiard_polygon, billiard_lorentz, triangular_billiard
...
function triangular_billiard(sides::Int, d_cell::Real, center = [0,0]; setting = "standard")
    bt = Obstacle[]
    theta = pi/2.

    r = d_cell/(2*cos(pi/sides))
    verteces = [[r*cos(2π*i/sides + theta), r*sin(2π*i/sides + theta)] + center for i in 1:sides]
    ...
    for i in eachindex(verteces)
        ...
        w = ending - starting
        if setting == "periodic"
            normal =  normalize([-w[2], w[1]])*d_cell
            wall = PeriodicWall(starting, ending, normal, wallname*" $i")
        else
            normal = [-w[2], w[1]]
            wall = FiniteWall(starting, ending, normal, wallname*" $i")
